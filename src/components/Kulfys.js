  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      ...
      if (this.state.intialCount <= kulfiesCount) {
        ...
        for (
          let i = kulfiesCount;
          i >= kulfiesCount - (this.state.intialCount + this.state.takeCount);
          i--
        ) {
          if (i > 0) {
            const kulfy = await this.state.kulfyV3.methods.kulfys(i).call();
            if (
              !this.state.kulfies.map((item) => item.id).includes(kulfy.id) &&
              kulfy.id != 0
            ) {
              this.setState({
        ...
        this.setState({
          intialCount: this.state.intialCount + this.state.takeCount,
        });
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="grid-item-details">
                            ...
                            <h6>
                              Tips Recieved:{" "}
                              {Web3.utils.fromWei(item.tipAmount, "ether")} ONE
                            </h6>
                            <a
                              href={"/kulfy?id=" + item.id}
